...
## 3. Visual & Aesthetic Architecture (Apple VisionOS / macOS Style)
- **`src/components/icons/SynIcons.tsx`:** Handcrafted SVG icons and Greek Sigma "Σ" / Cinema infinity convergence logo mark.
- **`src/components/ShaderCanvas.tsx`:** 60fps WebGL fluid gradient shader background (zero external 3D engine overhead, high performance across Safari, Chrome, Firefox, Mobile, iOS, and Android).
...
- **`src/components/LiquidGlassFilters.tsx`:** Procedural `<feTurbulence>` and `<feDisplacementMap>` SVG optical refraction and 35mm film grain filters.
- **Typography & Dark Palette:** Plus Jakarta Sans with SF Pro / Inter font stack and Cosmic Obsidian base (`#030611`).
...
- `src/components/Lobby.tsx`: Greek Syn-themed watchroom creation and joining flow.
